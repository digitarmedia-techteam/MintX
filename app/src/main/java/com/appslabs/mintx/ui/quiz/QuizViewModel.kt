package com.appslabs.mintx.ui.quiz
...
import com.appslabs.mintx.data.repository.QuizRepository
import kotlinx.coroutines.launch

class QuizViewModel(private val repository: QuizRepository, private val questionTracker: com.appslabs.mintx.utils.QuestionTracker) : ViewModel() {
    ...
    private val _scoreUpdateEvent = com.appslabs.mintx.utils.SingleLiveEvent<Int>()
    ...
    fun fetchQuestions(categories: List<String> = emptyList()) {
        ...
        viewModelScope.launch {
            ...
            try {
                // Ensure we have the latest XP before calculating question count
                val uid = com.google.firebase.auth.FirebaseAuth.getInstance().currentUser?.uid
                if (uid != null) {
                    val xp = repository.getUserXP(uid)
                    _totalXP.value = xp
                ...
                // Calculate question count based on user level
                val currentXP = _totalXP.value ?: 0L
                val levelInfo = com.appslabs.mintx.utils.LevelUtils.calculateLevelInfo(currentXP)
                val questionCount = com.appslabs.mintx.utils.LevelUtils.calculateQuestionCount(levelInfo.level)
                
                android.util.Log.d("QuizViewModel", "User XP: $currentXP, Level: ${levelInfo.level}, Question Count: $questionCount")
                
                // Pass categories and question count to repository
                // Repository will return questions with proper difficulty distribution (60% Easy, 20% Medium, 20% Hard)
                val result = repository.getQuestions(categories, questionCount)
                
                if (result.isNotEmpty()) {
                    _questions.value = result // Already the right count with proper distribution
                    _currentIndex.value = 0
                    _userAnswers.value = mutableMapOf()
                    _currentScore.value = 0
                    _quizFinished.value = false
    ...
    fun selectAnswer(questionIndex: Int, answerKey: String) {
        ...
        if (isCorrect) {
            ...
            // Track as correct - never show again
            questionTracker.markQuestionCorrect(question.id)
        } else {
            _currentScore.value = currentScoreVal - 1
            _scoreUpdateEvent.value = -1
            // Track as wrong - show after 50 questions
            questionTracker.markQuestionWrong(question.id)
        }
        
        // Increment counter for every question attempted
        questionTracker.incrementCounter()
    }
    ...
    fun nextQuestion() {
        ...
        val currentAnswers = _userAnswers.value ?: emptyMap()
        
        // If question was not answered (skipped), still increment counter
        if (current < questions.size && !currentAnswers.containsKey(current)) {
            questionTracker.incrementCounter()
        }
